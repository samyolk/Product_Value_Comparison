    ...
    while not number:
        try:
            ...
            while number < 0:
                print(error)
                number = float(input(question))
            max_budget = round(number, 2)
            return print(output, max_budget)
        except ValueError:
            print(error)
...
def get_product_details():
    ...
    while not number:
        try:
            ...
            product_amount = (round(number * conversion_rate, 2))
        except ValueError:
            print(error)
...
while loop is True:
    ...
    get_product_details()
    all_product_details.append([product_name, product_amount, product_price])
...
print("\n----------------------------------------")
print("All the products you are comparing:")
while product_number < len(all_product_details):
    print(all_product_details[product_number][0])
    print(all_product_details[product_number][2])
    print()
    product_number += 1

product_number = 0
while product_number < len(all_product_details):
    product_unit_prices.append(all_product_details[product_number][2]
                               / all_product_details[product_number][1])
    product_number += 1
print("\nThe average product unit price: ", sum(product_unit_prices)
      / len(product_unit_prices))

product_number = 0
cheapest_product_price = all_product_details[product_number][2]
while product_number < len(all_product_details):
    if cheapest_product_price > all_product_details[product_number][2]:
        cheapest_product_price = all_product_details[product_number][2]
        cheapest_product = product_number
    product_number += 1
product_number -= 1
print("\nThe cheapest product is ", all_product_details[cheapest_product][0],
      "at $", all_product_details[cheapest_product][2])

product_number = 0
most_expensive_product_price = all_product_details[product_number][2]
while product_number < len(all_product_details):
    if most_expensive_product_price < all_product_details[product_number][2]:
        most_expensive_product_price = all_product_details[product_number][2]
        most_expensive_product = product_number
    product_number += 1
product_number -= 1
print("The most expensive product is ",
      all_product_details[most_expensive_product][0],
      "at $", all_product_details[most_expensive_product][2])
...
if all_product_details[recommended_product][2] > max_budget:
    print("\nSorry, there is no recommended product as all of their product "
